...
The two-column mode depends on a mapping convention between narrative sections and viewer components. Viewers component declarations appear directly after a paragraph, the framework identifies the active narrative region and updates the right-column viewer accordingly. This interaction layer includes JavaScript (scrollama) responsible for observing scroll position, managing active states, switching or updating iframe viewers, and sending commands between the host page and the embedded components.
...
This section hierarchy is especially important in two-column scrollytelling mode. The active narrative region is determined in relation to these generated section blocks, and viewer state can be associated with a particular section or subsection. As the reader scrolls through the document, StoryKit can use the hierarchical section structure to determine which viewer should be displayed, which animation or command should be triggered, and which part of the narrative is currently active.
